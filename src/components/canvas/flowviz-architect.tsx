...
export default function FlowVizArchitect({
  ...
  // Handle auto-generation from landing page with 3-state logic
  useEffect(() => {
    ...
    try {
      ...
      if (idToUse) {
        ...
        if (response.ok) {
          ...
          if (newFlowchart.id) {
            // Use replaceState to update URL without triggering a full re-mount
  ...
  const generateDiagram = async (e?: React.FormEvent, customTopic?: string) => {
    ...
    try {
      ...
      // Clear localStorage flags only after successful generation
